...
class ALFAIntent:
    def __init__(self):
        ...
        self.intents = {
            ...
            "DESFAZER": ["desfazer", "desfaÃ§a", "volte atras"],
            "SELECIONAR_TUDO": [
                "selecionar tudo",
                "selecione tudo",
                "marcar tudo",
    ...
    def direction(self, text):
        ...
        # Variacoes que o reconhecimento de voz pode produzir
        ...
        if any(x in c for x in [
            "cima", "acima", "topo", "suba", "sobe", "eleva"
        ]):
            return "cima"

        if any(x in c for x in [
            "baixo", "abaixo", "fundo", "desca", "baixe", "desce"
        ]):
            return "baixo"
    ...
    def parse(self, text):
        ...
        # Comando de mouse inequÃ­voco: nÃ£o depende do LLM.
        mouse_words = [
...
def brain_fallback(text):
    # SÃ³ para perguntas abertas. Nunca para comandos bÃ¡sicos.
        ...
        "messages": [
            {
                "role": "system",
                "content": (
                    "VocÃª Ã© o cÃ©rebro conversacional do ALFA. "
                    "Responda em portuguÃªs do Brasil, de forma curta, "
                    "natural, objetiva e coerente. "
                    "VocÃª NÃƒO controla mouse, teclado ou programas. "
                    "Comandos de computador sÃ£o executados pelo ALFA."
                ),
            },
            {
                "role": "user",
    ...
    try:
        ...
        return "NÃ£o consegui processar isso agora."
...
def execute(intent, voice):
    ...
    if kind == "MOVER_MOUSE":
        ...
        if move_mouse(direction):
            ...
            voice.speak(
                "Entendi que vocÃª quer mover o mouse. "
                "Para qual lado?"
            )
        return True
    ...
    if kind == "POSICAO_MOUSE":
        x, y = pyautogui.position()
        voice.speak(f"O mouse estÃ¡ em {x} por {y}.")
        return True
    ...
    if kind == "ABRIR":
        ...
        if program and open_program(program):
            ...
            voice.speak("NÃ£o consegui abrir esse programa.")
        return True
    ...
    if kind == "FECHAR":
        ...
        if program and close_program(program):
            ...
            voice.speak(f"{program or 'Esse programa'} nÃ£o estÃ¡ aberto.")
        return True
...
def main():
    print("=" * 65)
    print("                         ALFA PRO")
    print("             VOZ + NLU SEMÃ‚NTICO + AÃ‡ÃƒO LOCAL")
    print("=" * 65)
    ...
    while True:
        try:
            ...
            print("INTENÃ‡ÃƒO:", intent)

            if execute(intent, voice):
                continue

            # Somente aqui entra o cÃ©rebro conversacional.
